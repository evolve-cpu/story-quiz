...
//   const { completeQuiz } = useQuizProgress(); // ✅ use hook
...
//   useEffect(() => {
//     gsap.set([scene25Ref.current, scene25TextBoxRef.current], { opacity: 0, scale: 1 });
...
//     tl.to(scene25Ref.current, { opacity: 1, duration: 1, ease: "power2.out" })
//       .to(scene25TextBoxRef.current, { opacity: 1, duration: 1 }, "<0.5")
//       .to(scene25Ref.current, { scale: 1.05, duration: 2 })
//       .to(scene25TextBoxRef.current, { opacity: 0, duration: 1 }, "-=0.5")
//       .to(scene25Ref.current, {
...
//         onComplete: () => {
//           console.log("✅ Final module complete → marking quiz as complete");
//           completeQuiz(); // ✅ call the hook function
//         }
//       });
...
//         }`}
//       >
//         <p>gibbo and tilly wave goodbye to each other</p>
//       </div>
//     </div>
//   );
// };
...
const Module17 = () => {
  ...
  useEffect(() => {
    ...
    tl.to(scene25Ref.current, { opacity: 1, duration: 1, ease: "power2.out" })
      ...
      .to(scene25Ref.current, { scale: 1.05, duration: 2 })
      .to(scene25TextBoxRef.current, { opacity: 0, duration: 1 }, "-=0.5")
      .to(scene25Ref.current, { opacity: 0, duration: 1 })
      // ✅ Ensures dispatch happens when timeline reaches end
      .call(() => {
        console.log("✅ Final module complete → marking quiz as complete");
        dispatch({ type: actions.SET_QUIZ_COMPLETE });
      });
